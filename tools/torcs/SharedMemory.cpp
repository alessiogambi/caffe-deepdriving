...
CSharedMemory::CSharedMemory() :
		pMemory(0), SharedMemoryID(-1), IsDataUpdated(false) {
	attach();
	initMemory();
...
void CSharedMemory::attach() {
	SharedMemoryID = shmget((key_t) 4567, sizeof(SharedMemoryLayout_t),
			0666 | IPC_CREAT);

	if (SharedMemoryID == -1) {
		std::cerr
				<< "ERROR: Cannot attach to shared memory from TORCS (ID is invalid)!"
				<< std::endl;
	} else {
		...
		if (pMemory == ((void*) -1)) {
			pMemory = 0;
			std::cerr
					<< "ERROR: Cannot attach to shared memory from TORCS (Memory address is invalid)!"
					<< std::endl;
		} else {
			std::cout << std::endl
					<< "********** Memory sharing started, attached at "
					<< pMemory << " **********" << std::endl;
		}
	}
}
...
void CSharedMemory::detach() {
	if (pMemory) {
		if (shmdt(pMemory) == -1) {
			std::cerr << "ERROR: Cannot detach from shared memory!"
					<< std::endl;
		} else {
...
void CSharedMemory::initMemory() {
	if (pMemory) {
		...
		Indicators.DistanceToLeftMarkingOfCenterLane = -Indicators.LaneWidth
				/ 2;
		Indicators.DistanceToRightMarkingOfCenterLane = Indicators.LaneWidth
				/ 2;
...
void CSharedMemory::readDrivingAction() {
	SharedMemoryLayout_t * const pShared = (SharedMemoryLayout_t *) pMemory;
// If pShared->written == 0 the control is on the Environemt
	std::cout << "Written ?" << pShared->written << std::endl;
...
//		Image.readFromMemory(pShared->data, TORCS_IMAGE_WIDTH,
//		TORCS_IMAGE_HEIGHT, RESIZE_IMAGE_WIDTH, RESIZE_IMAGE_HEIGHT);
...
//  pShared->steerCmd = TorcsData.Steering;
	...
	pShared->speed = TorcsData.Speed;

	// This shall convert the current image, if any to its serialized form
	// FIXME Most likely this is broken, since we are passing around smaller images from the DB.
	Image.writeToMemory(pShared->data, TORCS_IMAGE_WIDTH, TORCS_IMAGE_HEIGHT); //TORCS_IMAGE_WIDTH, TORCS_IMAGE_HEIGHT);
//  uint8_t * pMemory, int SourceWidth, int SourceHeight
//  Image.readFromMemory(pShared->data, TORCS_IMAGE_WIDTH, TORCS_IMAGE_HEIGHT, RESIZE_IMAGE_WIDTH, RESIZE_IMAGE_HEIGHT);
//  Image.writeToMemory(Image.readFromMemory(TORCS_IMAGE_WIDTH, TORCS_IMAGE_HEIGHT, RESIZE_IMAGE_WIDTH, RESIZE_IMAGE_HEIGHT));
	std::cout << "Sending Image and Speed." << TorcsData.Speed << std::endl;
	std::cout.flush();

	// Gives the control to the AI
	if (IsDataUpdated) {
		pShared->written = 1;
		// Local not sure if needed
		IsDataUpdated = false;
	}
